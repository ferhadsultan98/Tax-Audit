...
const LatesUpdatesSection = () => {
  ...
  useEffect(() => {
    fetch("http://172.20.10.91:8000/api/blog/posts/")
      .then((res) => res.json())
      .then((data) => {
          ...
          {blogPosts.map((post) => (
            <article key={post.id} className={`blogCard ${post.featured ? 'featured' : ''}`}>
              <div className="cardImage">
                <img src={post.image_id ? `http://172.20.10.91:8000/api/blog/images/${post.image_id}/` : post.image} alt={post[`title_${language}`] || post.title} />
